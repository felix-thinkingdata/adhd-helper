...
[功能特色](#功能特色) · [安装](#安装) · [从源码构建](#从源码构建) · [本地正式安装](#本地正式安装) · [使用文档](docs/USER_GUIDE.md) · [架构文档](docs/ARCHITECTURE.md)
...
用一条细主线贯穿任务、时间和动作。Focus 不抢注意力，只把当前主线放在最容易回来的位置，像一个温和的外部大脑。
...
Tauri 的 npm 包和 Rust crate 需要保持兼容版本。项目已在 `package.json` 中锁定 Tauri 相关 npm 包版本；如果出现版本不匹配，先执行 `npm install`，不要把这些版本改回宽泛的 `^2`。
...
```bash
cd /Users/felix/app/WebstormProjects/adhd-helper
npm install
npm run tauri -- build
```
...
打开后把 **Focus.app** 拖到 **Applications**。如果旧版正在菜单栏运行，先右键菜单栏图标退出，再启动新版。

也可以直接打开 `.app` 产物：

```bash
open src-tauri/target/release/bundle/macos/Focus.app
```
...
| 后端 | Tauri 2.10.x (Rust) | 计时器、提醒、通知在后端线程运行 |
